...
| `SC-FacePlus.glb` | Generated in code by `Generate Sample Characters` (Tool B) | `CC0-1.0` | SC-Face (same blendshapes, jaw joint, eye bones, `aa`/`happy` mask + `Smile` mapping) plus a UV-transform texture expression on a distinct, CPU-readable lit material. Emits `KHR_character_expression` with `_joint`, `_mask`, `_mapping`, AND `_texture` (+ `KHR_animation_pointer` / `KHR_texture_transform`). |
| `SC-Body.glb` | Generated in code by `Generate Sample Characters` (Tool B) | `CC0-1.0` | Synthetic T-pose humanoid skeleton (unique neutral bone names) with a skeleton mapping identified by a versioned Unity Humanoid vocabulary URI, a TPose reference pose, and a portrait camera hint. Emits `KHR_character` + `KHR_character_skeleton_mapping` + `KHR_character_reference_pose` + `KHR_node_camera_hint`. Bones only (no skinned mesh). |
| `SC-LookAt.glb` | Generated in code by `Generate Sample Characters` (Tool B) | `CC0-1.0` | Minimal passive carrier for `KHR_node_lookat_target`: a `KHR_character` root whose `LookAtTargetSet` marks two nodes (one carries a `hint`, one is hint-less). No expressions, gaze consumer, or skeleton. Emits `KHR_character` + `KHR_node_lookat_target`. |
...
| `SC-PseudoVRM.glb` | Generated in code by `Generate Sample Characters` (Tool B) | `CC0-1.0` | The SC-Partial body whose exported GLB is post-processed to inject **synthetic** `VRMC_vrm` / `VRMC_springBone` tokens into `extensionsUsed` + a stub root extension, so it *reads* like a VRM-origin asset. **Not a real VRM** (the tokens are empty stubs). Proves that an imported expression asset fails closed instead of being silently re-exported through a lossy controller projection. |
| `SC-Degraded.glb` | Generated in code by `Generate Sample Characters` (Tool B) | `CC0-1.0` | The SC-Body humanoid, retaining its versioned Unity Humanoid vocabulary URI, whose exported GLB is post-processed to omit the `leftFoot` role association. This is valid `KHR_character_skeleton_mapping` data—the extension defines no required anatomy—but the optional Unity Humanoid adapter classifies the selected mapping as incomplete, yielding `SkeletonMapping: Degraded`. The only committed fixture that exercises the Degraded tri-state (loaded by the Health demo picker). Emits `KHR_character` + `KHR_character_skeleton_mapping` + `KHR_character_reference_pose` + `KHR_node_camera_hint`. |
...
real VRM data; `SC-Degraded` is `SC-Body` with its `leftFoot` role association omitted.) They are dedicated to the
